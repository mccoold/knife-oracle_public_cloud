    ...
    class OpcDbcsCreate < Chef::Knife
      ...
      def run
        attrcheck = {
                     ...
                     'ssh user identity file'   => options[:identity_file]
                    }
        ...
        if valid.at(0) == 'true'
          ...
          createcall = dbcscreate.create(data_hash, "#{config[:id_domain]}", "#{config[:user_name]}", "#{config[:passwd]}")
          if createcall.code == '401' or createcall.code == '404'
            print ui.color('ERROR!!!', :red, :bold)
            print ui.color(createcall.body, :red)
          else
            res = JSON.parse(dbcscreate.create_status(createcall['location'], "#{config[:id_domain]}",
            ...
            result = result.inst_list("#{config[:id_domain]}", "#{config[:user_name]}",
                                      "#{config[:passwd]}", "#{res['service_name']}")
